 ...
 *
 * */
public class ServerSysLog {
    public static void main(String[] args) {
        new Thread(){
            @Override
            public void run() {
                ServerSysLog server = new ServerSysLog();
                server.Server();
            }
        }.start();
    }
    ...
    public void Server(){
        try {
            ...
            while(true){
                 ...
                 * socket.send(packet);
                 * */
                socket.send(new DatagramPacket(msg.getBytes(),msg.length(), InetAddress.getByName("159.226.16.157"),5141));
                socket.send(new DatagramPacket(msg.getBytes(),msg.length(), InetAddress.getByName("192.168.16.4"),5140));
            }
        } catch (Exception e) {
            e.printStackTrace();
